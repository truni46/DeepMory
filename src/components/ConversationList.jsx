    ...
    return (
        <div className="px-2 space-y-1 overflow-y-auto custom-scrollbar">
            {conversations.map((conv) => (
                <div
                    key={conv.id}
                    className={`group relative flex items-center justify-between px-3 py-2 rounded-lg cursor-pointer transition-colors ${activeId === conv.id
                            ? 'bg-bg-tertiary'
                ...
                >
                    ...
                    <button
                        ...
                        className="opacity-0 group-hover:opacity-100 p-1 hover:bg-bg-main rounded transition-opacity flex-shrink-0"
